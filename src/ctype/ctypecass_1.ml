...
type cassUuidGen
(* type uuid
module Mod : Ctypes_types.TYPE with type 'a typ = uuid = struct  end;; *)

type uuid
let uuid : uuid structure typ = structure "CassUuidGen";
...
let ml_cass_cluster_new = foreign "cass_cluster_new" (void @-> returning ptr uuid);
let ml_cass_session_new = foreign "cass_session_new" (void @-> cassSession);
